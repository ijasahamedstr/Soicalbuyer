...
function User({ userdata }) {
  ...
  useEffect(() => {
    ...
    if (userdata) {
      fetchUserDetails();
      const intervalId = setInterval(fetchUserDetails, 500000000); // Fetch user details every 5 seconds
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        setUserDetails(response.data); // Update user details with the response data
      } else {
        Swal.fire({
          icon: 'error',
          title: 'Error!',
